...
public class Utilidades {
	...
	public String validarFormatoGenerar(GenerarFactura genera) {
		try {			
			if(genera != null) {
				if(genera.getFclientes()  != null) {
					if(genera.getTickets() != null) {
						if(genera.getTickets().size() > 0) {
							String validarDatosCliente= validarDatosCliente(genera.getFclientes());
							if(validarDatosCliente.equals("")) {								
								return validarFormatoTickets(genera.getTickets());
							}else {
						...
						return "Se requiere los Tickets a facturar tickets:[Object]";
					}
				} else {
					return "Se requieren los datos de Cliente que Factura fclientes:Object";
				}
			} else {
				return "Se requiere un formato valido {fclientes:Object, tickets:[Object]}";
			}
		} catch (Exception e) {
			return "Error al validarFormatoGenerar";
		}
	}
